# 执行机构模型与重新调参对异构无人平台编队控制器排序的影响

英文题目：Effects of actuation models and retuning on controller rankings in heterogeneous vehicle formation control
...
本稿采用上面的最新英文题目。`Journal_figure_reproduction.zip` 提供本次 JBSMSE 主文和补充材料的全部16幅整图绘制脚本及冻结输入。解压到独立目录，保留子目录结构，安装 `baseline/requirements.txt` 中的依赖，然后在解压目录运行 `python -B generate_jbsmse_artwork.py`。这一步不运行新实验、不重新拟合统计量。`READ_ME_FIRST_JBSMSE.md` 提供文章、作者、单位及使用说明。

原 `baseline-figures`、`latest-figures` 命令及仓库旧预览保留早期版本的图号与版式，不是当前稿件的最终图件入口。实验记录、数值与科学限制保持不变。当前校验清单覆盖新增文件；旧归档内部的标题和说明是对应冻结版本的记录。
